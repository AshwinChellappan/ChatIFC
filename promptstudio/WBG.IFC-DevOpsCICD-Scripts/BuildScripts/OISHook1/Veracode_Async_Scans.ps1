...
Write-Host "[INFO] Downloading veracode scanner......................."
$download_url = "https://search.maven.org/remotecontent?filepath=com/veracode/vosp/api/wrappers/vosp-api-wrappers-java/24.4.13.0/vosp-api-wrappers-java-24.4.13.0-dist.zip"
$output_file = 'Veracode_wrappers.zip'
$Response = Invoke-WebRequest -Uri $download_url -OutFile $output_file
$StatusCode = $Response.StatusCode
Write-Host "[INFO] Veracode scanner file downloaded...$output_file"
...
$authCheckCmd = "java -jar $javaWrapper -action GetUserInfo -username itosc_lob -vid $apiId -vkey $apiKey "


try {
    $output = Invoke-Expression $authCheckCmd

    if ($output) {
        Write-Output "[INFO] Auth check response Present"
    } else {
        Write-Output "[INFO] No Auth check response was received."
    }


    if ($output -like "*Error*" -or $output -like "*credentials are not in the correct format*") {
        Write-Output "[INFO] - API ID and Key validation failed: $output"
    } else  
      
    {
        Write-Output "[INFO] - API ID and Key are valid."
    }
} catch {
    Write-Output "[INFO] - Auth Key Validation Failed."
...
Function Get-ScanName($scan) {
    ...
    Write-Host "[INFO] Invoking Scanning Fucntion......................."
...
Function Assert-AppIdExists($app) {
    ...
    Try {
        ...
        Write-Host "[INFO] App Details Found, App Name: $app"
        Write-Host "[INFO] App Details Found, App ID: $appId"
...
Function Get-Provisional($app,$casenumber,$hookusername,$hookpassword,$hookname) {
...
Write-Host "[INFO] Invoking Provisional Validator......................."
...
Function getsummery(){
    ...
    write-host "[Info] Downloading the summary results ......................."
    [string]$randomName = Get-Random
...
Function main()
{
    ...
    Try{
         Write-Host "[INFO] Case - Provisional validation......................."

        $provisional = Get-Provisional $app $casenumber  $hookusername $hookpassword $hookname
        if ($provisional[0] -ne $null -and $provisional[1] -eq "go"){
            ...
            Write-Host "[INFO] Task Initiated......................."
