...
class TaskService:
    ...
    def update_task_progress(self, instance_id, space_instance, increment = None, reset = False):
        if increment is not None:
            self.increment = increment
        # Check reset
        if reset:
            space_instance['ingestionProgress'] = self.increment
        else:
            space_instance['ingestionProgress'] += self.increment

        # Update status and handle the float error
        if round(space_instance['ingestionProgress'], 1) == 1 or space_instance['ingestionProgress'] > 1:
            current_status = STATUS_MSG.COMPLETED
            space_instance['ingestionProgress'] = 1
        else:
            current_status = STATUS_MSG.IN_PROGRESS
        space_instance['ingestionStatus'] = current_status
