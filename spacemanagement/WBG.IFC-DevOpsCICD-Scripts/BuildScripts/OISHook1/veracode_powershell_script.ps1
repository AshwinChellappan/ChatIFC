...
$download_url = "https://search.maven.org/remotecontent?filepath=com/veracode/vosp/api/wrappers/vosp-api-wrappers-java/20.12.7.3/vosp-api-wrappers-java-20.12.7.3-dist.zip"
...
Function Start-VeracodeScanNoWait($app, $sandbox, $file, $scan) {
    ...
    If (-not ([string]::IsNullOrEmpty($sandbox))) {
        ...
        $result = java -jar $javaWrapper -vid $apiId -vkey $apiKey $proxyInfo -action uploadandscan -appname $app -createprofile false -filepath "$file" -version "$scan" -scanallnonfataltoplevelmodules true
    ...
    Write-host  "[Info] Pre Scan Initiated ......................."
       ...
       $summary_status = getsummery
            ...
            elseif ( $summary_status -eq "Conditional Pass") {
                ...
                Write-Host "[Error] Exception: Pipeline task error, Please re-run the build again or contact oisdevsecopssupport1@worldbankgroup.org "
                exit 1 }

            } }

        Catch{
        Write-Host "[Error] Exception in Pipeline task, Please re-run the build again or contact oisdevsecopssupport1@worldbankgroup.org "
        exit 1
        }
}
